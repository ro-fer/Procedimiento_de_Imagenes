...
O = zeros(size(l));
P = false(size(l)); %matriz lógica (me indica si un pixel fue o no procesado), false = cero, true = 1.
...
for k = 3:2:Smax
Zmin = ordfilt2(I,1,ones(k),'symmetric');%busca el mínimo dentro de la ventana kxk
Zmax = ordfilt2(I,k*k,ones(k),'symmetric'); %busca el máximo (kxk) dentro de la venta
